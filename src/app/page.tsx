...
function FullscreenOverlay({ label }: { label: string }) {
  return (
    <div className="fixed inset-0 z-[9999] flex h-screen w-screen items-center justify-center bg-app-bg px-4">
...
export default function Home() {
  ...
  const { user, isAdmin, loading } = useAuth(); // isAdmin should be boolean | null

  // treat "admin status not decided yet" as still loading (prevents flicker)
  ...
  if (shouldRedirect) {
    return <FullscreenOverlay label="Redirecting..." />;
  }
  ...
  //  Logged in but NOT admin => don't show login card
  return (
    <div className="min-h-screen flex items-center justify-center bg-app-bg px-4">
